    ...
    for i in range(0, len(filelist)):
        ...
        if os.path.isfile(filepath):
            ...
            for map in testlist:
                ...
                res.append(map)
            file.close()
    return X1, X2, T, L1, L2, LT, Y, res


# shape=[batch_size, None]
def make_batches(data, batch_size):
    X1, X2, T, L1, L2, LT, Y, res = data
    ...
    t = np.array(res[: batch_size * num_batches])
    t = np.reshape(t, [batch_size, num_batches])
    t_batches = np.split(t, num_batches, axis=1)
    return list(zip(data_batches1_rs, data_batches2_rs, data_batchesT_rs, len_batches1, len_batches2, len_batchesT, label_batches, t_batches))
...
def run_epoch(session, model, batches, step):
    ...
    for x1, x2, t, l1, l2, lt, y, res in batches:
...
def test_epoch(session, model, batches, step):
    ...
    total_FP = 0
    total_FN = 0
    for x11, x21, t1, l11, l21, lt1, y1, res1 in batches:
        score, loss = session.run([model.cos_score, model.loss_op],
                                feed_dict={model.input1: x11, model.input2: x21, model.inputT: t1, model.len1: l11, model.len2: l21, model.lent: lt1,
                                           model.target: y1})
        temp.append(loss)
        total_correct = total_correct + get_correct(score, y1, res1)
        index = index + 1
...
def get_correct(score, target, testData):
    result = 0
