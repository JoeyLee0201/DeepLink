...
wordModel = word2vec.Word2Vec.load('test/nocode%s.model' % NAME)
...
def main():
    train_batches = make_batches(read_data(path='./train%s' % NAME), BATCH_SIZE)
    test_batches = make_batches(read_data(path="./testset%s" % NAME), BATCH_SIZE)
